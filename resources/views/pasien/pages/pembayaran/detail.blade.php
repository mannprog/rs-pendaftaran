@extends('pasien.layouts.app', ['title' => 'Detail Pembayaran'])
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if ($data->bukti_pembayaran !== null)
                        @if ($data->status === 0)
                            <a href="#" class="btn btn-sm btn-success shadow disabled">Sudah Dikonfirmasi</a>
                        @else
                            <a href="#" class="btn btn-sm btn-secondary shadow disabled">Sudah Upload</a>
                        @endif
                    @else
                        <button type="button" class="btn btn-sm btn-primary shadow mb-4" data-bs-toggle="modal"
                            data-bs-target="#upload{{ $data->id }}"><i class="ti ti-upload"></i>
                            Upload</button>
                        @include('pasien.pages.pembayaran.component.upload')
                    @endif
                </div>
                <div class="card-body">
                    @if ($data->bukti_pembayaran == null)
